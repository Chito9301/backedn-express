...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res) => {
    ...
    const media = await Media.find({ uploadedBy: req.user!.id }).sort({ createdAt: -1 }).skip(skip).limit(limit)
...
router.get(
  "/:id",
  asyncHandler(async (req, res) => {
    const media = await Media.findById(req.params.id).populate("uploadedBy", "username avatar")
